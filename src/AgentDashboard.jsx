...
function AgentDashboard({ token, onNotify, openProfileNonce }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // Nudge newly added agents (empty phone or company) to finish their
        // profile before they start listing homes.
        if (!savedProfile.phone?.trim() || !savedProfile.company?.trim()) {
          setShowProfile(true)
  ...
  const saveProfile = async (event) => {
    event.preventDefault()
    try { setProfile(await updateProfile(profile, token)); setShowProfile(false); setProfileNudge(false); onNotify('Agent profile updated.') } catch (error) { onNotify(error.message) }
  }
